...
const Dashboard = () => {
  ...
  const handleConfirm = async (eventId) => {
    try {
      ...
      const res = await axios.post(BASE_URL + "/events/publish/" + eventId);
          ...
          data[selectedService]?.map((singleData) => {
            return (
              <ServiceCard
                ...
                isPublished={singleData && singleData?.item?.isPublished}
                status={singleData && singleData?.item?.status}
                description={singleData && singleData?.item?.description}
